...
enum ScramState {
    Init,
    SentChallenge { initial_client_message: Vec<u8>
        ...
        match self.state {
            ScramState::Init => {
                ...
                match self.channel_binding {
                    ChannelBinding::None | ChannelBinding::Unsupported => { // Not supported.
                        if gs2_header[0] != 0x79 { // ord("y")
                ...
                let frame = parse_frame(&rest).map_err(|_| "can't decode initial message".to_owned())?;
                ...
                server_nonce += &generate_nonce().map_err(|_| "failed to generate nonce".to_owned())?;
            ...
            ScramState::SentChallenge { ref server_nonce
                                      , ref identity
                                      , ref salted_password
                                      , ref gs2_header
